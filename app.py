...
# --- 2. THE ULTIMATE MODERN CORPORATE LASER THEME (CUSTOM CSS) ---
modern_laser_css = """
<style>
/* Clean & Deep Minimalist Dark Background */
.main {
...
/* Elegant, High-End Minimal Title with Single Laser Underline */
.modern-title {
...
/* Premium Matte Cards with Sharp Cyber Laser Border */
.laser-card {
...
/* Professional Solid High-Tech Button */
...
with col1:
    ...
    # Sliders and selectors with better default spread for dynamic percentages
...
with col2:
    ...
    if st.button("RUN PREDICTIVE ANALYSIS"):
        try:
            # Dynamic matching to prevent flat 0% error
            input_dict = {}
            ...
            fig = go.Figure(go.Indicator(
                ...
                gauge={
                    ...
                    'bar': {'color': "rgba(0,0,0,0)"}, # Core thick bar hidden
                    'bgcolor': "#0b0c10",
                    'borderwidth': 1,
                    'bordercolor': "rgba(255,255,255,0.1)",
                    'steps': [
                        {'range': [0, 40], 'color': 'rgba(0, 243, 255, 0.1)'},   # Light Laser Cyan
                        {'range': [40, 70], 'color': 'rgba(255, 170, 0, 0.1)'},  # Soft Warning Orange
                        {'range': [70, 100], 'color': 'rgba(255, 0, 85, 0.1)'}   # Muted Danger Red
                    ],
                    'threshold': {
                        'line': {'color': "#ff007f", 'width': 5}, # Pure sharp glowing needle
                        'value': probability
                    }
                }
            ))
            ...
            # Clean Modern Outlined Result Bars
            if prediction == 1 or probability > 50:
                st.markdown(f"<div style='border: 1px solid #ff0055; box-shadow: 0 0 10px rgba(255,0,85,0.2); border-radius: 6px; padding: 15px;'><h4 style='color:#ff0055; text-align:center; margin:0; font-weight:600; letter-spacing:1px;'>⚠️ SYSTEM ALERT: HIGH CHURN RISK DETECTED</h4></div>", unsafe_allow_html=True)
            else:
                st.markdown(f"<div style='border: 1px solid #00ffcc; box-shadow: 0 0 10px rgba(0,255,204,0.2); border-radius: 6px; padding: 15px;'><h4 style='color:#00ffcc; text-align:center; margin:0; font-weight:600; letter-spacing:1px;'>✅ STATUS STABLE: CUSTOMER RETENTION PROBABLE</h4></div>", unsafe_allow_html=True)
